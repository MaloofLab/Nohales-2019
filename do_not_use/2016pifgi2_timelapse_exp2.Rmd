...
p.growthrate
ggsave(p.growthrate,file="Marian_pif3gi3_timelapse_v2.png",dpi=300,width=11,height=8) # needs to clean up graph
...
p.growthrate <- p.growthrate +
  geom_smooth(mapping=aes(color=genotype),
              stat="smooth",
              fill="grey20",
              method="loess",span=.07,n=200, se = FALSE) # need to play with span and n
p.growthrate <- p.growthrate + theme(
  axis.text.x=element_text(size=18),
  #axis.ticks = element_blank(),
  panel.background = element_rect(fill = "white")) + 
...
#p.growthrate <- p.growthrate + coord_cartesian(ylim=c(-0.001,0.005))
p.growthrate
ggsave(p.growthrate,file="Marian_pif3gi3_timelapse_v3.png",dpi=300,width=11,height=8) 
```
