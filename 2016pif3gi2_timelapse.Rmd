...
p.growthrate <- p.growthrate + coord_cartesian(xlim=100:max(tl.rates3$after.stratification_hr)) # + coord_cartesian(ylim=c(-0.001,0.005))
p.growthrate <- p.growthrate + scale_color_manual(values = colors, guide=FALSE)
p.growthrate
...
p.growthrate <- p.growthrate + coord_cartesian(xlim=c(100,max(tl.rates3$after.stratification_hr)))
...
p.growthrate
ggsave(p.growthrate,file="Maria_pif3gi2_timelapse_v3.png",dpi=300,width=6,height=4) 
```
