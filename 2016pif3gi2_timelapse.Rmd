...
exp2.filename<-read.table("exp2.file.txt",skip=1)
exp2.filename$frame<-1:nrow(exp2.filename)
...
hyp.data[hyp.data$exp==1,"file"]<-vlookup(hyp.data[hyp.data$exp==1,"frame"],exp1.filename[,2:1],2)
# obtaining file date/time (exp2)
hyp.data[hyp.data$exp==2,"file"]<-vlookup(hyp.data[hyp.data$exp==2,"frame"],exp2.filename[,2:1],2)
...
```{r}
# plot (height version)
p.height<-ggplot(hyp.data,aes(x=after.stratification_hr,y=pixel,color=as.factor(rep),shape=factor(exp))) + geom_point()+facet_grid(genotype~.)
